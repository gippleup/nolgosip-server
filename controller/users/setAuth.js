...
module.exports = async (req, res) => {
  ...
  if (!auth || !email) return res.end('INVALID INPUT');
  if (auth !== 'manager' && auth !== 'user') return res.end('INVALID AUTH TYPE');

  const sessionAuth = await utils.jwt.verify(req.session.auth, (err, decoded) => {
    if (err) return false;
    return decoded.data;
  });
  if (!sessionAuth || sessionAuth === 'user') return res.end('UNAUTHORIZED REQUEST');
  ...
  if (!existingUser) return res.end('NO SUCH USER');
